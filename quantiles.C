...
void quantiles() {
   ...
   TH1F *h = new TH1F("h","demo quantiles",2,-1,0);
   ...
   legend->Draw();
	...
	for(int i =0;i<32;i++)
	{
		cout<<yq[i]<<","<<yq[i+68]<<"   "<<xq[i+68]-xq[i]<<"   "<<yq[i+68]-yq[i]<<endl;
		if(yq[i+68]-yq[i]<temp){temp=yq[i+68]-yq[i];tempi=i;}
	}
	cout<<endl<<endl<<temp<<"  "<<tempi<<endl;

	cout<<endl<< h->GetMaximumBin()<<endl;
}
